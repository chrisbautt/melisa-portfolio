    ...
    return (
        <>
            <details className="list-none border border-[#e1e1e1] rounded-[15px] md:hover:border-[#a855f7] shadow-[5px_5px_10px] shadow-[#eeeeee] transition-all duration-300">
                <summary className="flex justify-between items-center gap-4 text-lg font-bold py-6 px-4 cursor-pointer" onClick={toggleOpen}>
                    {question}
                    <Image src={`/icons/${isOpen ? "keyboard_arrow_up" : "keyboard_arrow_down"}.svg`} alt="" width={40} height={40} className="w-[40px] h-[40px]" />
                </summary>
